        ...
        *,
        ...
        raise WorkspaceRuntimeConfigError(
            ...
            hint=(
                ...
                f"prompts default to '{_DEFAULT_PROMPT_FILE}'"
            ),
        )
        ...
        *,
        ...
        if "prompt" in value:
            raise WorkspaceRuntimeConfigError(
                ...
                message="inline prompt content is not allowed; use AGENTS.md or prompt_file",
            )

        prompt_file = _optional_str(value=value, key="prompt_file", path=f"{base_path}.prompt_file")
        if prompt_file is None:
            prompt_file = _DEFAULT_PROMPT_FILE

        prompt = resolved_prompts_by_path.get(prompt_file)
        if prompt is None and prompt_file != _DEFAULT_PROMPT_FILE:
            prompt = resolved_prompts_by_path.get(_DEFAULT_PROMPT_FILE)

        if prompt is None:
            raise WorkspaceRuntimeConfigError(
                code="workspace_reference_missing",
                path=f"{base_path}.prompt_file" if "prompt_file" in value else _DEFAULT_PROMPT_FILE,
                message=(
                    f"resolved prompt for '{prompt_file}' was not found; expected workspace root "
                    f"'{_DEFAULT_PROMPT_FILE}'"
                ),
            )

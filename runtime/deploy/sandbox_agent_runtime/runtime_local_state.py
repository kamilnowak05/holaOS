...
def get_workspace(workspace_id: str, *, include_deleted: bool = False) -> WorkspaceRecord | None:
    with runtime_db_connection() as conn:
        row = conn.execute("SELECT * FROM workspaces WHERE id = ? LIMIT 1", (workspace_id,)).fetchone()
    if row is None:
        return None
...
def create_workspace(
    *,
    ...
    record = get_workspace(resolved_workspace_id, include_deleted=True)
    if record is None:  # pragma: no cover
        raise RuntimeError("failed to load created workspace")
    return record
...
def update_workspace(workspace_id: str, **fields: Any) -> WorkspaceRecord:
    ...
    with runtime_db_connection() as conn:
        sql = "UPDATE workspaces SET " + ", ".join(assignments) + " WHERE id = ?"  # noqa: S608
        conn.execute(sql, values)
    updated = get_workspace(workspace_id, include_deleted=True)
    if updated is None:  # pragma: no cover
        raise RuntimeError("failed to load updated workspace")
    return updated

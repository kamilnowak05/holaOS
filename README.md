...
- `runtime/deploy/sandbox_image/` from `hola-boss-ai`
...
- `runtime/`: current OSS candidate surface, starting from `deploy/sandbox_image`
- `docs/`: allowlist and standalone-boundary audit
- `scripts/`: sync/import helpers

## Current Finding

`deploy/sandbox_image/sandbox_agent_runtime/*` has no direct Python imports from `hola-boss-ai/src`.

That means the first OSS extraction step is:
1. keep `deploy/sandbox_image` intact
